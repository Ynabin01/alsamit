...
class HomeController extends Controller
{
    public function index(){
       ...
       if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%about%")->where('page_type','Group')->latest()->first()!=null){
        ...
        if(Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%client%")->where('page_type','Group')->latest()->first()!=null){
            $client_id = Navigation::query()->where('nav_category','Home')->where('nav_name', 'LIKE', "%client%")->where('page_type','Group')->latest()->first()->id;
            $client = Navigation::query()->where('parent_page_id',$client_id)->latest()->get();
            //return $partners;
        }
        else{
            $client = null;
        ...
        //return $missons;       
        return view("website.index")->with(['client'=>$client,'newsbanner'=>$newsbanner,'job_categories'=>$job_categories,'testimonial'=>$testimonial,'statistics'=>$statistics,'banners'=>$banners,'partners'=>$partners,'about'=>$About,'menus'=>$menus,'global_setting'=>$global_setting,'sliders'=>$sliders,'missons'=>$missons,'message'=>$message,'jobs'=>$jobs]);
    }
